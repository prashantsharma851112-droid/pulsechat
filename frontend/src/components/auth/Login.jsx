...
import { Zap, Lock, AtSign, Mail, ArrowLeft, RefreshCw, AlertCircle } from 'lucide-react';
...
export default function Login({ switchToRegister }) {
  ...
  // Unverified account OTP interception state
  ...
  useEffect(() => {
    if (!needsOtp && googleClientId && !googleClientId.includes('sample') && window.google?.accounts?.id) {
      try {
        ...
        {!needsOtp ? (
          <div>
            <div style={{ textAlign: 'center', marginBottom: '1.5rem' }}>
              <div style={{ background: 'var(--accent)', width: '54px', height: '54px', borderRadius: '16px', display: 'flex', alignItems: 'center', justifyContent: 'center', margin: '0 auto 0.75rem auto', boxShadow: '0 8px 20px rgba(99, 102, 241, 0.3)' }}>
                <Zap size={28} color="#fff" />
              </div>
              <h2 style={{ fontSize: '1.65rem', fontWeight: 700, margin: '0 0 0.35rem 0', color: 'var(--text-main)' }}>Welcome to PulseChat</h2>
              <p style={{ color: 'var(--text-muted)', fontSize: '0.88rem', margin: 0 }}>Sign in with Google or Email</p>
            </div>
            ...
            <div style={{ display: 'flex', alignItems: 'center', margin: '1rem 0 1.25rem 0' }}>
              ...
              <span style={{ padding: '0 10px', fontSize: '0.78rem', color: 'var(--text-muted)', textTransform: 'uppercase', letterSpacing: '0.05em' }}>or with credentials</span>
              <div style={{ flex: 1, height: '1px', background: 'var(--border)' }}></div>
            </div>
          ...
          <form onSubmit={handleVerifyOtp}>
            <button
              ...
              onClick={() => { setNeedsOtp(false); setError(''); setSuccessMsg(''); }}
            ...
            <div style={{ textAlign: 'center', marginBottom: '1.5rem' }}>
              ...
              <h2 style={{ fontSize: '1.4rem', fontWeight: 700, margin: '0 0 0.4rem 0', color: 'var(--text-main)' }}>Email Verification Required</h2>
